...
`stay-ops-container`의 관리자 백엔드 서버입니다.
...
```text
src/main/java/com/cj/stayops/backend
├── StayOpsServerApplication.java
└── config
    └── SecurityConfig.java
...
참고:
- 현재 기준으로 Java 패키지 경로도 `stayops` 기준으로 정리되어 있습니다.
...
```env
SPRING_DATASOURCE_URL=jdbc:postgresql://localhost:5432/stayops
SPRING_DATASOURCE_USERNAME=stayops
SPRING_DATASOURCE_PASSWORD=stayops1234
SERVER_PORT=8080
```
